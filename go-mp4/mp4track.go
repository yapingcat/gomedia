...
func (track *mp4track) writeH264(h264 []byte, pts, dts uint64) (err error) {
	...
	codec.SplitFrameWithStartCode(h264, func(nalu []byte) bool {
		...
		case codec.H264_NAL_SPS:
			...
			loc, sc := codec.FindStartCode(nalu, 0)
			nalu_without_start := nalu[loc+int(sc):]

			spsid := codec.GetSPSIdWithStartCode(nalu)
			for _, sps := range h264extra.spss {
				loc, sc := codec.FindStartCode(sps, 0)
				sps_without_start := sps[loc+int(sc):]
				if spsid == codec.GetSPSIdWithStartCode(sps) {
					is_append = false
					if bytes.Equal(sps_without_start, nalu_without_start) {
						return true
					}
				}
			}
		...
		case codec.H264_NAL_PPS:
			...
			for _, pps := range h264extra.ppss {
				loc, sc := codec.FindStartCode(pps, 0)
				pps_without_start := pps[loc+int(sc):]

				if ppsid == codec.GetPPSIdWithStartCode(pps) {
					is_append = false
					if bytes.Equal(pps_without_start, nalu_without_start) {
						return true
					}
				}
			}
